...
from process_data.datatypes.match_choice import MatchChoice
from process_data.datatypes.utils import to_data
from process_data.match_items.process_match import find_existing_match


@dataclass(frozen=True)
class RecordedDupes:
    """Known duplicates and likely non-duplicates"""

    dupes: defaultdict[str, set[str]]

    def get_dedupe_map(self) -> Mapping[str, str]:
    ...
    def from_data(cls, data: Any, skip_updates: bool = False) -> RecordedDupes:
        ...
        dupes: defaultdict[str, set[str]] = defaultdict(
            set,
            {key: {str(v) for v in val} for key, val in data["dupes"].items()},
        )

        if not skip_updates:
            handle_overlaps(dupes)

        return cls(dupes=dupes)
...
def handle_overlaps(dupes: defaultdict[str, set[str]]) -> None:
...
) -> Mapping[tuple[str, str], AbstractSet[str]]:
...
def unify_overlapped_values(
    dupes: defaultdict[str, set[str]],
    all_overlaps: Mapping[tuple[str, str], AbstractSet[str]],
) -> None:
    """Unify items that are duplicated as values"""
...
def unify_key_value_overlaps(
    dupes: defaultdict[str, set[str]],
    overlapping_dedupes: set[str],
) -> None:
    """Unify items that are duplicated as keys and values"""
